-- wybranie imion i nazwisk znajomych gdzie ID usera = 1 #użycie w moich znajomych i profilu
...
-- wybranie imion i nazwisko znajomych, którzy wysłali zaproszenie dla usera o ID = 2 #użycie w wysłanych zaproszeniach
...
-- wybranie spotkan dla usera od ID = 2 oraz dacie za max tydzień #użycie w najbliższych spotkaniach
...
-- wybranie imion i nazwisko użytkoników, których user od ID = 1 nie ma w znajomych oraz nie otrzymał od nich zaproszenia #użycie w inni użytkownicy w panelu znajomych
...
-- wybranie tytułów i dat spotkań z "numerami" tygodni gdzie uczestnikiem jest user o ID = 2 #użycie na stronie głównej
SELECT meetings.title, meetings.date, TIMESTAMPDIFF(week,CURRENT_TIMESTAMP,meetings.date) AS `WOFBYTODAY` FROM meetings, meetings_members WHERE MONTH(CURRENT_TIMESTAMP) = MONTH(meetings.date) AND meetings_members.ID_user = 2 AND meetings_members.ID_meeting = meetings.ID ORDER BY WOFBYTODAY, meetings.date